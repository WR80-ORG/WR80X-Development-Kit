...
char* replace(const char* token, const char* old_substr, const char* new_substr) {
    ...
    // Se token aponta para dentro do buffer est�tico, precisamos copi�-lo
    // para um local tempor�rio antes de potencialmente free() o buffer.
    char *local_copy = NULL;
    const char *src = token;
    if (buffer != NULL && token >= buffer && token < buffer + buffer_capacity) {
        local_copy = strdup(token);   // cria c�pia independente do token
    ...
    if (buffer_capacity < new_size) {
        if (buffer) {
            free(buffer); // agora seguro: src j� � c�pia se apontava para buffer
            buffer = NULL;
            buffer_capacity = 0;
        }
        buffer = (char*)malloc(new_size);
        if (!buffer) {
            // cleanup da c�pia tempor�ria se existia
    ...
    int prefix_len = (int)(pos - src); // pos aponta dentro de src (n�o em token, se copiamos)
    ...
    // liberamos a c�pia tempor�ria porque j� copiamos tudo para buffer
    if (local_copy) {
        free(local_copy);
        local_copy = NULL;
    }
...
char* get_code(const char* beg_cmd, const char* end_cmd) {
    char* code = NULL;
    int total_size = 0;
    int depth = 1; // j� estamos dentro do bloco principal
    linenum++;
    linebegin = linenum;
    char* line_tmp = NULL;
...
bool skip_block(const char* begin, const char* end) {
    if (strcmp(token, begin) == 0) {
        int depth = 1; // j� estamos dentro de um rep
        while (fgets(line, sizeof(line), fileopened)) {
        	line_to_upper();
            linenum++;		// 6
            ...
            if (strcmp(token, begin) == 0) {
                ...
                if (depth == 0) {
                    break; // este � o endp que fecha o bloco inicial
                }
            }
        }
...
char** parse_parameters(int *argc_out) {
    char **pvalues = NULL;
    *argc_out = 0; // zera o contador que ser� retornado

    // Continua a partir de onde strtok parou (ap�s _mov)
    //char *token = token;
    ...
    while (token != NULL) {
        ...
        // Realoca mais espa�o no vetor de ponteiros
        ...
        // Pr�ximo token separado por v�rgula
        token = strtok(NULL, ",");
    }
